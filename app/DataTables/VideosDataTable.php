...
class VideosDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('menu_active', function ($row) {
                $menus = '';
                foreach ($row->MenuVideo as $key => $value) {
            ...
            ->addColumn('action', function ($row) {
                ...
                if (Auth::user()->hasRole('Superadmin')) {
                    $form .= '
                    <form action="' . route('admin.videos.destroy', $row->id) . '" method="post" id="cancel_' . $row->id . '">';
                    $form .= csrf_field();
                ...
                $btn =   '
                    <div class="btn-group dropend">
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item" href="' . route('admin.videos.edit', $row->id) . '">Edit</a>
                            </li>
                            ' . $form . '
                        </ul>
                    </div>
    ...
    protected function getColumns()
    {
        return [
            Column::make('title'),
            Column::make('link')->title('Link Videos'),

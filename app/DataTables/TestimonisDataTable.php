...
class TestimonisDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('menu_active', function ($row) {
                $menus = '';
                foreach ($row->MenuTestimoni as $key => $value) {
            ...
            ->addColumn('action', function ($row) {
                ...
                if (Auth::user()->hasRole('Superadmin')) {
                    $form .= '
                        <form action="' . route('admin.testimoni.destroy', $row->id) . '" method="post" id="cancel_' . $row->id . '">';
                    $form .= csrf_field();
                ...
                $btn =   '
                    <div class="btn-group dropend">
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item" href="' . route('admin.testimoni.edit', $row->id) . '">Edit</a>
                            </li>
                            ' . $form . '
                        </ul>
                    </div>
            ...
            ->rawColumns(['status', 'published', 'action', 'menu_active', 'image'])
            // ->addIndexColumn()
            ->setRowId('id');
    }
     ...
     *
     * @param \App\Models\Testimoni $model

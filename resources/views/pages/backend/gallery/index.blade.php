...
@push('title')
    Gallery - {{ $galleryCategoryName }}
@endpush

@push('action')
    <button type="button" class="btn btn-danger waves-effect waves-light material-shadow-none me-2"
        onclick="handlerDeleteAllData()">
        <i class="ri-delete-bin-fill"></i> Delete All
    </button>
    <a class="btn btn-secondary" href="{{ route('admin.gallery-category.index') }}"><i class="ri-arrow-left-fill"></i> Back</a>
@endpush

@section('content')
    <div class="card">
        <div class="card-body">
            <input type="hidden" name="galleryCategoryId" id="galleryCategoryId" value="{{ $galleryCategoryId }}">
            <form action="{{ route('admin.gallery.store', $galleryCategoryId) }}" method="POST"
...
@push('js')
    <script>
        const galleryCategoryId = document.querySelector('#galleryCategoryId').value;
        ...
        const getHeroSliders = () => {
            ...
            let html = "";
            requestAjax(`{{ route('admin.gallery.all-data') }}`, {
                galleryCategoryId
            }, 'POST', 'JSON', function(
                response) {
                $(".gallery-sliders").html('');
            ...
            const sortable = new Sortable(document.querySelector('.gallery-sliders'), {
                ...
                onEnd: function(evt) {
                    ...
                    requestAjax(`{{ route('admin.gallery.update-position') }}`, {
                        positions
        ...
        const handlerDeleteAllData = () => {
            ...
            messageBoxBeforeRequest('Want to delete all?', 'Yes. Sure', 'No, Close').then((
                ...
                if (result.isConfirmed) {
                    requestAjax(`{{ url('/admin/gallery/destroy-all/${galleryCategoryId}') }}`, {},
                        'DELETE', 'JSON',
                        function(response) {
                            if (response.status) {
        ...
        const handlerDeleteData = (id) => {
            requestAjax(`{{ url('/admin/gallery/destroy/${id}') }}`, {},
                'DELETE', 'JSON',
                function(response) {
                    if (response.status) {
                        messageTopRight('success', response.message)

...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.hero-image-sliders.store') }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="card .mt-4">
        <div class="card-header">
            <h4 class="card-title mb-0">Hero Sliders</h4>
            <span class="text-muted">Draggable card to change position image</span>
        </div><!-- end card header -->

        <div class="card-body p-3">
            <div class="row align-items-center justify-content-center hero-sliders gap-3"></div>
        </div>
        <!-- end card body -->
    </div>
@endsection
    ...
    <script>
        ...
        Dropzone.options.imageUpload = {
            ...
            dictDefaultMessage: `<div class="mb-3"><i class="display-2 text-muted ri-image-add-fill"></i></div>
                                 ...
                                 <h6 class="text-muted">Recomendation Image Size 1920x1080px</h6>`,
            init: function() {
        ...
        const getHeroSliders = () => {
            $(".hero-sliders").html('');
            $(".hero-sliders").html(setLoading());
            let html = "";
            requestAjax(`{{ route('admin.hero-image-sliders.all-data') }}`, {}, 'GET', 'JSON', function(response) {
                $(".hero-sliders").html('');
                ...
                $(".hero-sliders").html(html)
            });

            // Initialize SortableJS
            const sortable = new Sortable(document.querySelector('.hero-sliders'), {
                animation: 150,
                onEnd: function(evt) {
                    ...
                    requestAjax(`{{ route('admin.hero-image-sliders.update-position') }}`, {
                        positions
        ...
        const handlerDeleteAllData = () => {
            ...
            messageBoxBeforeRequest('Want to delete all?', 'Yes. Sure', 'No, Close').then((
                ...
                if (result.isConfirmed) {
                    requestAjax(`{{ url('/admin/hero-image-sliders/destroy-all') }}`, {},
                        'DELETE', 'JSON',
                        function(response) {
                            if (response.status) {
        ...
        const handlerDeleteData = (id) => {
            requestAjax(`{{ url('/admin/hero-image-sliders/destroy/${id}') }}`, {},
                'DELETE', 'JSON',
                function(response) {
                    if (response.status) {
                        messageTopRight('success', response.message)
